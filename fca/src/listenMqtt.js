...
function getSeqID(http, api, ctx, globalCallback) {
  var form = {
  ...
  http
    ...
    .then(function (res) {
      ...
      if (!!res[res.length - 1].error_results) throw res[0].o0.errors;
      if (!res[res.length - 1].successful_results) throw { 
      ...
      if (res[0].o0.data.viewer.message_threads.sync_sequence_id) {
        ...
        ctx.stopListen = !1;
        return listenMqtt(http, api, ctx, globalCallback);
      } 
      throw { 
...
function parseDelta(http, api, ctx, v, globalCallback) {
        ...
        if (delta.deltaMessageReaction && ctx.globalOptions.listenEvents) {
          ...
          if (delta.deltaMessageReply.repliedToMessage) {
            ...
            http
              ...
              .then(utils.parseAndCheckLogin(ctx, http))
              .then(function (res) {
                if (!!res[res.length - 1].error_results) 
                  throw res[0].o0.errors;
                if (!res[res.length - 1].successful_results) 
  ...
  switch (v.delta.class) {
    ...
    case 'ForcedFetch':
      ...
      http
        ...
        .then(function (res) {
          if (!!res[res.length - 1].error_results) throw res[0].o0.errors;
          if (!res[res.length - 1].successful_results) throw { error: 'forcedFetch: there was no successful_results', res };
          var fetchData = res[0].o0.data.message;
...
function listenMqtt(http, api, ctx, globalCallback) {
  ...
  mqttClient.on('connect', function () {
    ...
    mqttClient.publish(topic, JSON.stringify(queue), { qos: 1, retain: !1 });
  ...
  mqttClient.on('message', function (topic, message) {
    ...
    if (topic == '/t_ms') {
      ...
      for (var i in Message.deltas) {
        ...
        parseDelta(http, api, ctx, { delta }, globalCallback);
      }
...
module.exports = function (http, api, ctx) {
  let globalCallback;
  return function listenMessage(callback) {
    class MessageEmitter extends EventEmitter {
      stopListening(stopCallback) {
        globalCallback = () => {}
        
        if (typeof stopCallback != 'function')
          stopCallback = message => this.emit('stop', message);
        ...
        if (ctx.mqttClient) {
          ...
          ctx.mqttClient.end(!1, function (...message) {
      ...
      restartListening(restartCallback) {
        if (typeof restartCallback != 'function')
          restartCallback = message => this.emit('restart', message);

        if (ctx.mqttClient) 
          ctx.mqttClient.end(!1, function (...message) {
            restartCallback(message);
            ctx.mqttClient = null;
            getSeqID(http, api, ctx, globalCallback);
          });
        else 
          getSeqID(http, api, ctx, globalCallback);
      }
    ...
    if (!ctx.firstListen || !ctx.lastSeqId)
      ...
      listenMqtt(http, api, ctx, globalCallback);
