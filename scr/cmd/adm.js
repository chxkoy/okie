module.exports = {
  ...
  let config = process.cwd() + "/data/akaneConfig.json";
  ...
  if (t1 === "list") {
    if (data.botAdmin.length === 0) return api.sendMessage("There's no admin to display.", event.threadID, event.messageID);
    let ms = "",
      msg = "",
      c = 0;
    var img = [];
    
    for (let i = 0; i < data.botAdmin.length; i++){
   //   const name = (await api.getUserInfoV2(data.admins[i])).name;
      //(await api.getUserInfo(id, true))[id].name
      const name = (await api.getUserInfo(data.botAdmin[i], true))[data.botAdmin[i]].name;
      //const name = nam.name;
      let avtPath = __dirname+`/cache/${i}.png`;

      const avt = (await axios.get(`https://graph.facebook.com/${data.botAdmin[i]}/picture?height=720&width=720&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`,{
  responseType: "arraybuffer"
})).data;
    fs.writeFileSync(avtPath, Buffer.from(avt, "utf-8"));
    img.push(fs.createReadStream(avtPath));
      c += 1
      ms += c + ". Name: "+name+"\n[ f ]: https://facebook.com/"+data.botAdmin[i]+"\n\n"
    }
    msg += "[ ADMIN LIST ]\n\n" + ms;
  ...
  if (t1 === "remove" || t1 === "-r" || t1 === "r") {
    if (!as.includes(event.senderID)) return api.sendMessage("You don't have permission to this command.", event.threadID, event.messageID);
    if (data.botAdmin.length === 0) return api.sendMessage("There's no admin to remove.", event.threadID, event.messageID);
    data.botAdmin.splice(data.botAdmin.indexOf(t2), 1);
    fs.writeFileSync(config, JSON.stringify(data, null, 2));
